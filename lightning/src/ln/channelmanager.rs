...
impl PendingAddHTLCInfo {
	fn htlc_previous_hop_data(&self) -> HTLCPreviousHopData {
		...
		HTLCPreviousHopData {
			...
			htlc_rgb_payment: self.prev_htlc_rgb_payment,
		}
	}
}
...
mod fuzzy_channelmanager {
	...
	pub struct HTLCPreviousHopData {
		...
		pub htlc_id: u64,
		pub htlc_rgb_payment: Option<(ContractId, u64)>,
	...
	L::Target: Logger,
{
		...
		L: Clone,
	{
		...
		ChannelManager {
			...
			pending_outbound_payments: OutboundPayments::new(new_hash_map(), logger.clone(), ldk_data_dir.clone()),
		...
		match decoded_hop {
			...
			onion_utils::Hop::TrampolineReceive { .. } | onion_utils::Hop::TrampolineBlindedReceive { .. } => {
				...
				create_recv_pending_htlc_info(decoded_hop, shared_secret, msg.payment_hash,
					...
					current_height, msg.rgb_payment)
			},
	...
	///
	/// This is similar to [`Self::send_payment_with_route`] but includes the preimage
	/// in the onion payload, which is required for the recipient to claim the payment.
	///
	/// LDK will not automatically retry this payment, though it may be manually re-sent after an
	/// [`Event::PaymentFailed`] is generated.
	#[rustfmt::skip]
	pub fn send_spontaneous_payment_with_route(
		&self, route: Route, payment_hash: PaymentHash, payment_preimage: PaymentPreimage,
		recipient_onion: RecipientOnionFields, payment_id: PaymentId,
	) -> Result<(), RetryableSendFailure> {
		let best_block_height = self.best_block.read().unwrap().height;
		let _persistence_guard = PersistenceNotifierGuard::notify_on_drop(self);
		self.pending_outbound_payments
			.send_spontaneous_payment_with_route(route, payment_hash, payment_preimage,
				recipient_onion, payment_id, &self.entropy_source, &self.node_signer,
	...
	pub fn forward_intercepted_htlc(
		&self, intercept_id: InterceptId, next_hop: NextHopForward<'_>, _next_node_id: PublicKey,
		amt_to_forward_msat: u64, payment_to_forward_rgb: Option<(ContractId, u64)>,
	) -> Result<(), APIError> {
		let _persistence_guard = PersistenceNotifierGuard::notify_on_drop(self);

		let outbound_scid_alias = match next_hop {
		...
		let pending_htlc_info = PendingHTLCInfo {
			...
			outgoing_rgb_payment: payment_to_forward_rgb,
			routing,
...
This indicates a bug inside LDK. Please report this error at https://github.com/lightningdevkit/rust-lightning/issues/new";
	...
	fn claim_funds_internal(
		...
		skimmed_fee_msat: Option<u64>, from_onchain: bool,
							...
							(
								Some(MonitorUpdateCompletionAction::EmitEventAndFreeOtherChannel {
									event: events::Event::PaymentForwarded {
										...
										inbound_amount_forwarded_rgb: inbound_forwarded_rgb_payment
											.map(|(_, a)| a),
										payment_hash,
									},
								...
								new_intercept_events.push_back((
									events::Event::HTLCIntercepted {
										...
										expected_outbound_amount_msat: pending_add
											...
											.outgoing_amt_msat,
										intercept_id,
										inbound_rgb_amount: pending_add
											.forward_info
											.ingoing_rgb_payment
											.map(|(_, a)| a),
										expected_outbound_rgb_payment: pending_add
											.forward_info
											.outgoing_rgb_payment,
										is_swap,
										prev_outbound_scid_alias,
					...
					MonitorEvent::HTLCEvent(htlc_update) => {
						...
						if let Some(preimage) = htlc_update.payment_preimage {
							...
							self.claim_funds_internal(
								...
								htlc_update.htlc_rgb_payment.map(|(_, a)| a),
								None,
								true,
								counterparty_node_id,
								funding_outpoint,
...
pub struct Bolt11InvoiceParameters {
	...
	/// The RGB contract ID specified in the invoice.
	...
	L::Target: Logger,
{
	fn read<Reader: io::Read>(
		...
		let pending_outbounds =
			OutboundPayments::new(pending_outbound_payments.unwrap(), args.logger.clone(), args.ldk_data_dir.clone());
