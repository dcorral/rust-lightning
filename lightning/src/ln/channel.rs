	...
	SP::Target: SignerProvider,
{
		...
		L::Target: Logger
	{
		...
		let mut commitment_data = context.build_commitment_transaction(self.funding(),
	...
	SP::Target: SignerProvider,
{
		...
		L::Target: Logger,
	{
		...
		let mut commitment_data = self.build_commitment_transaction(
			funding,
			transaction_number,
			&commitment_point,
			true,
	...
	<SP::Target as SignerProvider>::EcdsaSigner: EcdsaChannelSigner,
{
	...
	pub fn update_fulfill_htlc(
	...
	) -> Result<(HTLCSource, u64, Option<u64>, Option<Duration>, Option<(ContractId, u64)>), ChannelError> {
		...
		self.mark_outbound_htlc_removed(msg.htlc_id, outcome).map(|htlc| {
			(htlc.source.clone(), htlc.amount_msat, htlc.skimmed_fee_msat, htlc.send_timestamp, htlc.rgb_payment)
		})
	}
		...
		L::Target: Logger,
	{
		...
		let mut counterparty_commitment_tx = self
			.context
		...
		L::Target: Logger,
	{
		...
		if let Some((_, amount_rgb)) = rgb_payment {
			...
			if amount_rgb > local_rgb_amount {
				return Err((
					LocalHTLCFailureReason::RgbHTLCMaximum,
					format!(
	...
	where
		L::Target: Logger,
	{
		let mut commitment_data = self.context.build_commitment_transaction(
